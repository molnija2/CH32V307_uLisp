...
/*  Changes are made in following procedures:
...
object *Workspace WORDALIGNED;
	...
	object *Workspace WORDALIGNED = (object *)Bank1_SRAM1_ADDR;
	#else
...
void movepointer (object *from, object *to) {
   uintptr_t limit = ((uintptr_t)(from) - (uintptr_t)(Workspace))/sizeof(/*uintptr_t*/object);
...
void SDWriteInt (FIL *file, uint32_t data)
{
	UINT uint_result ;
  f_write(file, &data, 4, &uint_result);
}

uint32_t SDReadInt (FIL *file) {
	UINT uint_result ;
  uint32_t data ;

  f_read(file, &data, 4, &uint_result);
...
unsigned int saveimage (object *arg) {
  ...
  for (unsigned int i=0; i<imagesize; i++) {
    ...
    if(obj->type == ARRAY2)
    {
        ...
        f_write(&File, obj->cdr, descriptor->size*descriptor->element_size+sizeof(array_desc_t), &uint_result);
    }
  }
#endif
...
unsigned int loadimage (object *arg) {
  ...
  // Change for x86
  object *Wsp_begin = (object *)SDReadInt(&File);
  ...
  GlobalEnv = (object *)(SDReadInt(&File) + wsp_shift ) ;
  GCStack = (object *)(SDReadInt(&File) + wsp_shift ) ;
  for (unsigned int i=0; i<imagesize; i++) {
    object *obj = &Workspace[i];
  ...
  for (unsigned int i=0; i<imagesize; i++) {
    ...
    if(obj->type == ARRAY2)
    {
        ...
        f_read(&File, &descriptor, sizeof(array_desc_t), &uint_result);
        int fullsize = descriptor.size*descriptor.element_size + sizeof(array_desc_t) ;
        obj->pointer = (uintptr_t)malloc(fullsize);
        *(array_desc_t*)(obj->pointer) = descriptor ;
        uintptr_t array_pointer = (uintptr_t)(obj->pointer) + sizeof (array_desc_t) ;
        f_read(&File, (void*)array_pointer, descriptor.size*descriptor.element_size, &uint_result);
    }
  }
#endif
  fres = f_close(&File);
...
extern symbol_t Aref2_name ; // Change for ARRAY2
...
*/
//  argument int *aray2_t - Change for ARRAY2
...
{
	//fwrite(&c, 1 , 1, fp_SDp);
	f_write(&SDpfile, &c, 1, &uint_result);
}
...
object *edit (object *fun) {
  while (1) {
    ...
    else if (c == 'r') fun = read0(gserial);
    else if (c == '\n') { pfl(pserial); superprint(fun, 0, pserial); pln(pserial); }
    else if (c == 'c') fun = cons(read0(gserial), fun);
...
object *sp_push (object *args, object *env) {
  ...
  object **loc = place(second(args), env, &bit, NULL);  //  NULL - Change for ARRAY2
...
object *sp_pop (object *args, object *env) {
  ...
  object **loc = place(arg, env, &bit, NULL);  //  NULL - Change for ARRAY2
...
object *sp_incf (object *args, object *env) {
  int bit, ar2 = 0 ;
  object **loc = place(first(args), env, &bit, &ar2);  //  Change for ARRAY2
...
#ifdef DEF_ARRAY2
  if (array2p(arg) && array2_lenght(arg)) return number(array2_lenght(arg));  //  Change for ARRAY2
#endif
...
#ifdef DEF_ARRAY2
  else if (array2p(array))    //  CHANGE for ARRAY2
  {
...
object *fn_makunbound (object *args, object *env) {
  ...
  if (!symbolp(var)) error(notasymbol, var);
#ifdef DEF_ARRAY2
  if(var->type == ARRAY2 )   //  Change for ARRAY2
      delarray2(var) ;
#endif
...
object *fn_gc (object *args, object *env) {
  if (args == NULL || first(args) != NULL) {
    ...
    gc(args, env);
    unsigned long elapsed = /*micros()*/millis() - start;
...
object *fn_pinmode (object *args, object *env) {
  ...
  if (keywordp(arg)) pm = checkkeyword(arg);
    ...
    #endif
    else if (mode == 5) pm = MODE_INPUT_ANALOG;
    else if (mode == 6) pm = MODE_OUTPUT_ANALOG;
  } else if (arg != nil) pm = OUTPUT;
  if(pinMode(pin, pm)) return tee;
  return nil;
}
...
#define	DEF_TYPE_FILE	0x20
#define	DEF_TYPE_DIR	0x10


object *fn_directory (object *args, object *env) {
#ifdef sdcardsupport
  (void) env;
  int type = DEF_TYPE_FILE | DEF_TYPE_DIR ;  // Files and directories
  ...
  if(fres != FR_OK)
  {  pfstring("problem reading from SD card", pserial); return nil; }
...
"Returns t if the argument is a number.";
const char doc132[] = "(sfloat_t number)\n"
"Returns its argument converted to a floating-point number.";
...
unsigned int lookup_table1_size = arraysize(lookup_table) ;

#ifdef extensions
// Table cross-reference functions
//extern const tbl_entry_t *tables[];

extern tbl_entry_t* lookup_table2_ptr ;
extern unsigned int lookup_table2_size ;
extern const tbl_entry_t lookup_table2[] ;
//tbl_entry_t *tables[10] ;
const tbl_entry_t *tables[]= {lookup_table, lookup_table2_ptr};
...
object *eval (object *form, object *env) {
  ...
  if ((uintptr_t)StackBottom - (uintptr_t)&stackpos > (MAX_STACK*sizeof(object))) { Context = NIL; error2("stack overflow"); }
...
object *nextitem (gfun_t gfun) {
  ...
  // Parse reader macros
    ...
    if (ch == '\\') { // Character
      ...
      setflag(NOESC);
      object *result = eval(read0(gfun), NULL);
      clrflag(NOESC);
      return result;
    }
    else if (ch == '(') { LastChar = ch; return readarray(1, read0(gfun)); }
    else if (ch == '*') return readbitarray(gfun);
    else if (ch >= '1' && ch <= '9' && (gfun() & ~0x20) == 'A') return readarray(ch - '0', read0(gfun));
...
object *readrest (gfun_t gfun) {
  ...
  while (item != (object *)KET) {
    if (item == (object *)BRA) {
      ...
      item = cons(bsymbol(QUOTE), cons(read0(gfun), NULL));
...
object *read0 (gfun_t gfun) {
  ...
  if (item == (object *)DOT) return read0(gfun);
  if (item == (object *)QUO) return cons(bsymbol(QUOTE), cons(read0(gfun), NULL));
  return item;
}
...
void setup () {
  ...
  #else
	#ifdef BOARD_HAS_ExternalRAM
  	  Workspace = (object *)Bank1_SRAM1_ADDR;
	#endif
  #endif
  ...
  initgfx();
  initTouchscreen() ;
  pfstring("uLisp 4.7 ", pserial); pln(pserial);
}
